...
function saveGame(isGaveUp = false) {
  ...
  if (!currentGameData) {
    console.error(
      'Bullseye Tracker: Tentative de sauvegarde sans données de partie.'
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      "Bullseye Tracker: Sauvegarde de l'objet partie suivant :",
      newGame
    );
    ...
    chrome.storage.local.get(['games'], (result) => {
      ...
      chrome.storage.local.set({ games: games }, () => {
        ...
        chrome.storage.local.get(['apiUrl', 'apiToken'], (config) => {
          ...
          if (config.apiUrl) {
            console.log(
              `Bullseye Tracker: Syncing game to ${config.apiUrl}...`
            );
            ...
            console.log(
              'Bullseye Tracker: Sending payload (JSON):',
            ...
            fetch(config.apiUrl, {
              ...
              .then((res) => {
                if (res.ok)
                  console.log('Bullseye Tracker: Game synced successfully!');
                else console.error('Bullseye Tracker: Sync failed', res.status);
              })
              .catch((err) =>
                console.error('Bullseye Tracker: Sync error', err)
              );
          }
        });
      });
    ...
    for (const mutation of mutationsList) {
      if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
        mutation.addedNodes.forEach((node) => {
          ...
          if (
            node.querySelector(endScreenSelector) ||
            node.matches(endScreenSelector)
          ) {
            injectSaveButton();
          }
        });
...
function injectSaveButton() {
  ...
  const container =
    document.querySelector('.game-finished_container__TEK6Q') ||
  ...
  if (container) {
    ...
    btn.style.cssText =
      'background-color: #7c3aed; color: white; padding: 10px 20px; border-radius: 9999px; font-weight: bold; margin-top: 10px; cursor: pointer; border: none; z-index: 9999;';
    ...
    btn.onclick = () => {
      if (currentGameData) {
        ...
        alert(
          'No game data available to save. Please refresh the page to capture data if this is an old game.'
        );
      }
    };
...
async function mergeGameState(newState) {
      ...
      if (newState.players && Array.isArray(newState.players)) {
        currentGameData.players = newState.players.map((newP) => {
          const existingP = existingPlayers.find((ep) =>
            typeof ep === 'string'
              ? ep === newP.playerId
              : ep.playerId === newP.playerId || ep.nick === newP.playerId
          );
  ...
  if (currentGameData.rounds && currentGameData.rounds.length > 0) {
    const sortedRounds = [...currentGameData.rounds].sort(
...
function updateScoreFromGameData(gameData) {
  ...
  if (gameData.rounds && Array.isArray(gameData.rounds)) {
    totalScore = gameData.rounds.reduce((sum, round) => {
      const points =
        round.score && typeof round.score.points === 'number'
          ? round.score.points
          : 0;
      return sum + points;
    }, 0);
  ...
  if (lobbyData && lobbyData.players) {
    currentGameData.players.forEach((p) => {
      if (
        typeof p === 'object' &&
        p.playerId &&
        (!p.nick || p.nick === p.playerId || p.nick === 'Unknown')
      ) {
        const lobbyPlayer = lobbyData.players.find(
          (lp) => lp.playerId === p.playerId
        );
        if (lobbyPlayer && lobbyPlayer.nick) {
          p.nick = lobbyPlayer.nick;
          updated = true;
          console.log(
            `Bullseye Tracker: Enriched nick from Lobby for ${p.playerId} -> ${p.nick}`
          );
        }
      }
    });
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (userData && userData.nick) {
            ...
            updated = true;
            console.log(
              `Bullseye Tracker: Fetched nick for ${p.playerId} -> ${p.nick}`
            );
          }
        }
      } catch (err) {
        console.error(
          `Bullseye Tracker: Failed to fetch nick for ${p.playerId}`,
          err
        );
      }
    }
