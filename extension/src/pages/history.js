...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function applyFiltersAndRender() {
    ...
    let filteredGames = allGames.filter((game) => {
      ...
      // Filtre Date
      const dateMatch = new Date(game.date)
        .toLocaleString()
        .toLowerCase()
        .includes(dateValue);
      // Filtre Score
      ...
      switch (currentSort.column) {
        ...
        case 'players':
          ...
          if (typeof valA === 'object' && valA !== null)
            valA = valA.nick || valA.playerId || '';
          valA = (valA || '').toLowerCase();

          valB = b.players[0];
          if (typeof valB === 'object' && valB !== null)
            valB = valB.nick || valB.playerId || '';
  ...
  function renderTable(games) {
    if (!games || games.length === 0) {
      tableBody.innerHTML =
        '<tr><td colspan="9" style="text-align: center;">No matching games found.</td></tr>';
      return;
    }
