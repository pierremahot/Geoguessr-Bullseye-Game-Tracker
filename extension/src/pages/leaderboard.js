...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderMapLeaderboards(games) {
    ...
    const sortedMaps = Object.keys(gamesByMap).sort(
      (a, b) => gamesByMap[b].length - gamesByMap[a].length
    );
    ...
    for (const key of sortedMaps) {
        ...
        game.players.forEach((player) => {
          const name =
            typeof player === 'object' && player !== null
              ? player.nick || player.playerId
              : player;
          if (!playerStatsOnMap[name])
            playerStatsOnMap[name] = { gameCount: 0, totalScore: 0 };
          playerStatsOnMap[name].gameCount++;
          playerStatsOnMap[name].totalScore += game.score;
        });
      });
      const sortedPlayersOnMap = Object.entries(playerStatsOnMap)
        .map(([name, stats]) => ({
          name,
          ...stats,
          avgScore: stats.totalScore / stats.gameCount,
      ...
      gamesOnMap.forEach((game) => {
        if (game.players.length > 0) {
          ...
          if (!teamStatsOnMap[teamKey])
            teamStatsOnMap[teamKey] = { gameCount: 0, totalScore: 0 };
          teamStatsOnMap[teamKey].gameCount++;
          teamStatsOnMap[teamKey].totalScore += game.score;
        }
      });
      const sortedTeamsOnMap = Object.entries(teamStatsOnMap)
        .map(([name, stats]) => ({
          name,
          ...stats,
          avgScore: stats.totalScore / stats.gameCount,
      ...
      const mapCard = createMapLeaderboardCard(
        displayMapName,
        gamesOnMap.length,
        sortedPlayersOnMap.slice(0, 5),
        sortedTeamsOnMap.slice(0, 5)
      );
