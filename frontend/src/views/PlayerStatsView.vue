...
import { User, Trophy, Map as MapIcon, Calendar, TrendingUp, AlertTriangle } from 'lucide-vue-next';
  ...
  <div class="space-y-6">
    <header class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h2 class="text-3xl font-bold text-white" v-if="stats && stats.player_name">{{ stats.player_name }} Stats</h2>
      ...
      <div class="flex items-center gap-4 bg-gray-800/50 p-2 rounded-lg border border-gray-700">
        ...
        <select v-model="scoreType" class="bg-gray-700 border-none rounded px-3 py-1 text-sm text-white focus:ring-1 focus:ring-purple-500">
    ...
    <div v-else class="space-y-6">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-gray-800/50 p-6 rounded-xl border border-gray-700 h-full">
          ...
          <div class="space-y-3">
             <div v-for="(team, index) in stats.best_teams" :key="index" class="p-3 bg-gray-700/30 rounded-lg">
                <div class="flex justify-between items-start mb-2">
                  <div class="text-sm text-gray-300">{{ team.team_name }}</div>
                  <div class="text-green-400 font-bold">{{ Math.round(team.average_score).toLocaleString() }}</div>
                </div>
                <div class="text-xs text-gray-500">{{ team.games_played }} games</div>
             </div>
             <div v-if="stats.best_teams.length === 0" class="text-gray-500 text-sm text-center py-2">No data</div>
          </div>
        </div>
      </div>
