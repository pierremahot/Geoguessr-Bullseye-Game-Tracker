...
const scoreType = ref('personal'); // 'personal' | 'game'
...
const loadStats = async () => {
  ...
  try {
    ...
    params.score_type = scoreType.value;

    stats.value = await fetchPlayerStats(playerId, params);
  } catch (err) {
...
watch([excludeAbandons, mapFilter, scoreType], loadStats);

// Chart Data
const chartData = computed(() => {
  if (!stats.value || !stats.value.score_history)
  ...
  if (grouping.value === 'game') {
    ...
    if (metric.value === 'average') {
      ...
      Object.values(days).forEach((d) =>
        Object.keys(d.maps).forEach((m) => allMaps.add(m))
      );

      const datasets = Array.from(allMaps).map((map, i) => {
        // Generate consistent color based on map name
        const hue =
          (map.split('').reduce((a, c) => a + c.charCodeAt(0), 0) * 137) % 360;
  ...
  plugins: {
    ...
    tooltip: {
      callbacks: {
        label: (context) => {
          if (grouping.value === 'game') {
            const point = [...stats.value.score_history].reverse()[
              context.dataIndex
            ];
          ...
          if (grouping.value === 'day' && metric.value === 'average') {
            // Re-calculate count for tooltip
            // Note: context.raw is the y value (average). We need to access the original data.
            // Since we don't have easy access to 'days' object here without recalculating or storing it,
            // we can infer it or just use the index if we trust the order.
            // A cleaner way is to store 'days' in a computed property, but for now let's try to access it via data index.
            // Actually, we can't easily access 'days' here because it's inside the computed.
            // Let's just show the value. The user can see intensity.
            // OR, better: We can attach the count to the dataset in a custom property if Chart.js allows, or just rely on visual.
            // Let's try to be clever:
            // We can reconstruct the count from the opacity? No, that's hacky.
            // Let's just return the standard label for now, the color is the main request.
        ...
        afterLabel: (context) => {
          if (grouping.value === 'day' && metric.value === 'average') {
            // We need the count.
            // Let's assume we can't easily get it without refactoring 'days' out.
            // I will refactor 'days' calculation to a separate computed if needed,
            // but for this quick change, I'll skip the explicit count number in tooltip unless requested.
            // The user asked for COLORING.
            return '';
          }
        },
      },
  ...
  <div class="space-y-6">
    ...
    >
      <div class="flex items-center gap-4">
        ...
        <div>
          <h2
            ...
            v-if="stats && stats.player_name"
          >
            {{ stats.player_name }} Stats
          </h2>
          <h2 class="text-3xl font-bold text-white" v-else>Player Stats</h2>
          <p class="text-gray-400 font-mono text-sm">{{ playerId }}</p>
        </div>
      </div>
      ...
      >
        ...
        <select
          v-model="scoreType"
          class="bg-gray-700 border-none rounded px-3 py-1 text-sm text-white focus:ring-1 focus:ring-purple-500"
        >
          <option value="personal">Personal Score</option>
          <option value="game">Game Score</option>
        </select>
        <div class="h-4 w-px bg-gray-700"></div>
        <input
    ...
    <div
      v-else-if="error"
      class="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-xl"
    >
      {{ error }}
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="bg-gray-800/50 p-6 rounded-xl border border-gray-700">
          ...
          <div class="text-3xl font-bold text-blue-400">
            {{ Math.round(stats.total_duration / 60) }}m
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="space-y-6">
          <div class="bg-gray-800/50 p-6 rounded-xl border border-gray-700">
            ...
            <div class="space-y-3">
              <div
                v-for="country in stats.best_countries"
                :key="country.country_code"
                class="flex items-center justify-between p-3 bg-gray-700/30 rounded-lg"
              >
                <div class="flex items-center gap-3">
                  <span
                    :class="`fi fi-${country.country_code} rounded shadow-sm`"
                  ></span>
                  <span class="font-medium text-gray-200 uppercase">{{
                    country.country_code
                  }}</span>
                </div>
                <div class="text-right">
                  <div class="text-white font-bold">
                    {{ Math.round(country.average).toLocaleString() }}
                  </div>
                  <div class="text-xs text-gray-400">
                    {{ country.count }} rounds
                  </div>
                </div>
              </div>
              <div
                v-if="stats.best_countries.length === 0"
          ...
          <div class="bg-gray-800/50 p-6 rounded-xl border border-gray-700">
            ...
            <div class="space-y-3">
              <div
                v-for="country in stats.worst_countries"
                :key="country.country_code"
                class="flex items-center justify-between p-3 bg-gray-700/30 rounded-lg"
              >
                <div class="flex items-center gap-3">
              ...
              <div
                v-if="stats.worst_countries.length === 0"
                class="text-gray-500 text-sm text-center py-2"
              >
                No data
              </div>
        ...
        <!-- Best Teams -->
        <div
          class="bg-gray-800/50 p-6 rounded-xl border border-gray-700 h-full"
        >
          <h3
            class="text-lg font-semibold text-white mb-4 flex items-center gap-2"
          >
          ...
          <div class="space-y-3">
            <div
              v-for="(team, index) in stats.best_teams"
              :key="index"
              class="p-3 bg-gray-700/30 rounded-lg"
            >
              <div class="flex justify-between items-start mb-2">
                <div class="text-sm text-gray-300">{{ team.team_name }}</div>
            ...
            <div
              v-if="stats.best_teams.length === 0"
              class="text-gray-500 text-sm text-center py-2"
            >
              No data
            </div>
