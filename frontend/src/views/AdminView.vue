      ...
      <div class="bg-slate-800 rounded-xl p-6 border border-slate-700">
        ...
        <div class="mb-6 bg-slate-700/50 p-4 rounded-lg">
          ...
          <p class="text-sm text-gray-400 mb-4">
            Select a "Primary" player (the main identity) and an "Alias" player
            (to merge into the primary).
          </p>

          <div class="flex flex-col gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-300 mb-1"
                >Primary Identity (Target)</label
              >
              <select
              ...
              >
                <td class="p-3 text-white">
                  <div class="flex items-center gap-2">
                    <span v-if="player.primary_id" class="text-gray-500"
                      >↳</span
                    >
                    {{ player.name }}
                  </div>
                </td>
                <td class="p-3 text-mono text-xs text-gray-500">
                  {{ player.id }}
                </td>
                <td class="p-3">
                  <span
                    v-if="player.primary_id"
                    class="px-2 py-1 rounded text-xs bg-purple-900/50 text-purple-300 border border-purple-700"
                    >Alias</span
                  >
                  <span
                    v-else-if="player.aliases && player.aliases.length > 0"
                    class="px-2 py-1 rounded text-xs bg-green-900/50 text-green-300 border border-green-700"
                    >Primary</span
                  >
                  <span
                    v-else
                    class="px-2 py-1 rounded text-xs bg-slate-700 text-gray-400"
                    >Standard</span
                  >
                </td>
                <td class="p-3">
...
import {
  fetchAdminPlayers,
  linkPlayers as apiLinkPlayers,
  unlinkPlayer as apiUnlinkPlayer,
} from '../services/api';
...
const linkPlayers = async () => {
  ...
  try {
    ...
    alert(
      "Failed to link players. Ensure you aren't creating a circular link."
    );
  }
};
...
const availablePrimaries = computed(() => {
  ...
  return players.value.filter(
    (p) => !p.primary_id && p.id !== selectedAlias.value
  );
});
...
const availableAliases = computed(() => {
  ...
  return players.value.filter(
    (p) =>
      p.id !== selectedPrimary.value && (!p.aliases || p.aliases.length === 0)
  );
});
...
const canLink = computed(() => {
  return (
    ...
    selectedAlias.value &&
    selectedPrimary.value !== selectedAlias.value
  );
});
