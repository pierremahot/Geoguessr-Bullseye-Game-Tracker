...
const columns = [
  {
    ...
    cell: (info) =>
      h(
        'span',
        { class: 'text-gray-400 font-mono text-xs whitespace-nowrap' },
    ...
    cell: (info) => {
      ...
      return h(
        'a',
        {
          ...
          class:
            'font-mono text-xs text-blue-400 hover:text-blue-300 hover:underline select-all',
          title: gameId,
        },
          ...
          if (count > 1) {
            return h('div', { class: 'relative inline-flex' }, [
              ...
              h(
                ...
                {
                  class:
                    'absolute -top-2 -right-2 bg-gray-700 text-white text-[10px] font-bold px-1 rounded-full border border-gray-600 shadow-sm',
                },
                count
              ),
            ]);
  ...
  },
  {
    ...
    header: () =>
      h('div', { class: 'flex items-center gap-2' }, [
        h(Users, { class: 'w-4 h-4' }),
        'Players',
      ]),
    cell: (info) => {
      const players = info.getValue() || [];
      if (players.length === 0)
        return h('span', { class: 'text-gray-500 text-xs' }, 'Unknown');
        ...
        players.map((p) =>
          h(
            RouterLink,
            {
              to: `/player/${p.id}`,
              class:
                'text-white font-medium text-xs hover:text-blue-400 transition-colors',
            },
            () => p.name
          )
        )
        ...
        return h('div', { class: 'flex items-start gap-2' }, [
          h(
            RouterLink,
            {
              to: `/team/${teamId}`,
              class:
                'p-1 bg-gray-700 rounded hover:bg-gray-600 transition-colors text-purple-400',
    ...
    cell: (info) => {
      ...
      return isFinished
        ? h(
            ...
            {
              class:
                'inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-900/50 text-green-400 border border-green-800',
            },
            'Finished'
          )
        : h(
            ...
            {
              class:
                'inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-yellow-900/50 text-yellow-400 border border-yellow-800',
            },
            'Ongoing'
          );
    },
  ...
  },
  {
    ...
    cell: (info) =>
      h(
        'div',
        { class: 'text-right font-mono text-xs text-gray-400' },
    ...
    cell: (info) =>
      h(
        'button',
        {
          ...
          class:
            'p-2 text-gray-500 hover:text-red-500 transition-colors rounded hover:bg-red-500/10',
          title: 'Delete Game',
        },
...
<template>
  <div
    class="bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700 flex flex-col h-[calc(100vh-12rem)] w-full overflow-hidden"
  >
    <div
    ...
    >
      <h2 class="text-2xl font-bold text-white flex items-center gap-2">
        ...
        <span
          ...
          >({{ totalGames }} total)</span
        >
      </h2>
      <div v-if="loading" class="text-gray-400 text-sm animate-pulse">
        Loading...
      </div>
    </div>
